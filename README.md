# sequenza-utils-rs / bam2seqz (v0.1.5)
...
## Per-version benchmark history

- Benchmark index: `copilot-desk/benchmark-history.tsv`
- Release snapshot script: `scripts/benchmark_release_snapshot.sh`
- Expected cadence for each release cycle:
  1. tag release (`vX.Y.Z`),
  2. bump `Cargo.toml` to next version,
  3. run snapshot script to archive artifacts under `tmp/benchmarks/<version>/`,
  4. append history row in `copilot-desk/benchmark-history.tsv`.

Example:

```bash
scripts/benchmark_release_snapshot.sh --version 0.2.0 --tag v0.2.0
```
...
|---|---:|---:|---|
| bam_default | 2:24.19 | 0:36.34 | Default BAM-input baseline |
| pileup_default | 0:27.17 | 0:09.06 | Pileup-input mode |
| region_12 | 0:28.36 | 0:01.73 | Single explicit region |
| parallel_regions_p8 | 0:38.81 | 0:01.86 | 8 explicit regions, `--parallel 8` |
| auto_bin_single_output_p8 | N/A | 0:10.71 | Rust auto-binning (`--parallel 8`, no ranged `-C`, single output); Python does not support this mode |
| normal2 | 3:13.72 | 0:49.73 | BAM with `--normal2` |

This is the primary benchmark section for current release messaging.
...
Because of those correctness deltas, `samtools` remains the default backend in v0.1.5.
